tinymce.PluginManager.add('mathjax', function(editor, url) {
  ...
  let mathjaxClassName = (settings && settings.className) || "math-tex";
  let mathjaxTempClassName = mathjaxClassName + '-original';
  let mathjaxSymbols = (settings && settings.symbols)|| {start: '\\(', end: '\\)'};
  let mathjaxUrl = (settings && settings.lib) || null;
  let mathjaxConfigUrl = ((settings && settings.configUrl) || url + '/config.js') + '?class=' + mathjaxTempClassName;
...
obj.circularReference = editor.getDoc().defaultView;

// Remove circular reference before stringifying
const cleanObj = { ...obj };
delete cleanObj.circularReference;

const jsonString = JSON.stringify(cleanObj);

console.log(jsonString);
	  //console.log('editor data '+JSON.stringify(editor.getDoc().defaultView));
